...
def plot_fit_results(count_edges,photon_edges,observed_counts,observed_counts_err,model,save_name,fit_times):
    ...
    if 'ThermalEmission' in model_names and 'ThickTarget' in model_names:
        ...
        ax_dat.text(0.77,0.6,f'Temp = {np.round(model.temperature_0.value,1)} {model.temperature_0.unit}',transform=ax_dat.transAxes,fontsize=tfs,color=colour_thermal)
        ax_dat.text(0.77,0.55,f'EM = {np.round(model.emission_measure_0.value,3)} {r'$\mathrm{\times \: 10^{49} \: cm^{-3}}$'}',transform=ax_dat.transAxes,fontsize=tfs,color=colour_thermal)
        ax_dat.text(0.77,0.5,f'Index = {np.round(model.p_1.value,1)}',transform=ax_dat.transAxes,fontsize=tfs,color=colour_nonthermal)
        ax_dat.text(0.77,0.45,f'E_c = {np.round(model.low_e_cutoff_1.value,1)} {model.low_e_cutoff_1.unit}',transform=ax_dat.transAxes,fontsize=tfs,color=colour_nonthermal)
        ax_dat.text(0.77,0.4,f'e Flux = {np.round(model.total_eflux_1.value,1)} {r'$\mathrm{\times \: 10^{35} \: e \: s^{-1}}$'}',transform=ax_dat.transAxes,fontsize=tfs,color=colour_nonthermal)
    ...
    ax_dat.set_ylim(0.6*np.min(observed_counts.value),2*np.max(observed_counts.value))
    ...
    ax_rat.text(0.8,0.06,r'$\mathrm{\chi^{2}_{red} = }$'+str(chi_red),transform=ax_rat.transAxes,fontsize=18)
